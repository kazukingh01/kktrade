...
if __name__ == "__main__":
    ...
    if args.since is not None and args.until is not None:
        ...
        dfbase = DB_from.select_sql(f"select {','.join(args.pkey)} from {args.tbl} where unixtime >= {int(args.since.timestamp())} and unixtime <= {int(args.until.timestamp())};")
        dfwk   = DB_to  .select_sql(f"select {','.join(args.pkey)} from {args.tbl} where unixtime >= {int(args.since.timestamp())} and unixtime <= {int(args.until.timestamp())};")
    else:
    ...
    for index in tqdm(np.array_split(np.arange(dfbase.shape[0]), dfbase.shape[0] // args.num)):
        ...
        if len(args.pkey) == 1:
            sql  = f"select * from {args.tbl} where {args.pkey[0]} in (" + ",".join(dfwk[args.pkey[0]].astype(str).tolist()) + ");"
        else:

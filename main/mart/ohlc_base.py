    ...
    for i_date in range(1, len(list_dates)):
        ...
        for exchange in EXCHANGES:
            ...
            for interval in INTERVALS:
                ...
                ndf_tg         = np.arange(int((date_fr - datetime.timedelta(hours=1)).timestamp()) // interval * interval, int(date_to.timestamp()) // interval * interval, interval, dtype=int)
                ...
                ndf_tg  = np.arange(int(date_fr.timestamp()) // interval * interval, int(date_to.timestamp()) // (interval // DIVIDES) * (interval // DIVIDES), (interval // DIVIDES), dtype=int)
                ndf_idx = np.concatenate([np.repeat(ndf_sbl, ndf_tg.shape[0]).reshape(-1, 1), np.tile(ndf_tg, ndf_sbl.shape[0]).reshape(-1, 1)], axis=-1)
